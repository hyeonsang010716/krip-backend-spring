 ...
 * 반환 Object[]: [FeedPost, likeCount(Long), commentCount(Long), isLiked(Boolean)].
...
public interface FeedPostRepository extends JpaRepository<FeedPost, String> {
    ...
    List<Object[]> findRowByPostId(@Param("postId") String postId, @Param("viewerId") String viewerId);
    ...
    List<Object[]> findByOwnerFirstPage(@Param("ownerId") String ownerId,
    ...
    @Query(SELECT + "from FeedPost p "
            ...
            + "      and p.postId < :cursor)) "
            + "order by p.createdAt desc, p.postId desc")
    List<Object[]> findByOwnerAfterCursor(@Param("ownerId") String ownerId,
                                          @Param("visibilities") Collection<FeedVisibility> visibilities,
                                          @Param("viewerId") String viewerId,
